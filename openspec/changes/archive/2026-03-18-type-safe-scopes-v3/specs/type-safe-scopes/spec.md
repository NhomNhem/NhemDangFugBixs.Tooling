...
The system SHALL provide a generic attribute `[AutoRegisterIn(typeof(TScope))]` for type-safe scope registration where `TScope` must inherit from `VContainer.Unity.LifetimeScope`.

#### Scenario: User applies attribute with valid scope type
- **WHEN** user writes `[AutoRegisterIn(typeof(GameplayLifetimeScope))]` on a class
...
- **WHEN** user writes `[AutoRegisterIn(typeof(string))]` on a class
...
- **WHEN** user writes `[AutoRegisterIn(typeof(NonExistentScope))]` on a class
...
- **THEN** the generator treats it identically to `[AutoRegisterIn(typeof(GameplayLifetimeScope))]`
