...
- Fix: Unity UPM compatibility — pinned Docusaurus dependency in package.json to an exact semver and updated website/package-lock.json so Unity Package Manager can install the runtime package from Git. This also resolves CI failures where `npm ci` failed due to package.json/package-lock.json mismatch.

### Notes

- Release v6.0.2 has been published with the analyzer DLLs attached as release assets. The runtime package is available via the `deploy` branch (UPM-friendly).
