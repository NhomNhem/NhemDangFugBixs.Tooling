...
public static class RegistrationEmitter {
    public static string GenerateSource(IEnumerable<ServiceInfo> services) {
        ...
        writer.WriteLine();
        writer.WriteLine("// NOTE: This generated code uses VContainer.Lifetime enum.");
        writer.WriteLine("// User code annotates classes with NhemDangFugBixs.Attributes.Lifetime,");
        writer.WriteLine("// which maps to VContainer.Lifetime values (Singleton, Transient, Scoped).");
        writer.WriteLine();
