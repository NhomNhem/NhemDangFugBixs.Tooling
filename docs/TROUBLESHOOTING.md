...
### Generator Diagnostics (NDFGxxx)

These diagnostics are generated by the source generator during code generation.

| Diagnostic | Issue | Quick Fix |
|------------|-------|-----------|
| [NDFG001](#ndfg001-attributes-missing) | Attributes assembly not found | Re-import package from branch=deploy |
| [NDFG002](#ndfg002-version-mismatch) | Attribute version mismatch | Update package |
| [NDFG003](#ndfg003-unsupported-asmdef) | Unsupported asmdef setup | Check package layout |
| [NDFG004](#ndfg004-generator-init-failed) | Generator failed to initialize | Check Unity logs |

---

### NDFG001: Attributes Missing
...
**Warning Message:**
```
NhemDangFugBixs.Attributes assembly not found. Code generation degraded.
...
**Problem:**
The generator cannot find `NhemDangFugBixs.Attributes` assembly. This can happen when:
1. Package not properly imported
2. asmdef references are broken
...
2. Remove existing NhemDangFugBixs entry
3. Add fresh: `https://github.com/NhomNhem/NhemDangFugBixs.Tooling.git?path=/&branch=deploy`

**Step 2: Verify asmdef references**
Ensure your asmdef includes reference to NhemDangFugBixs.Attributes (auto-added by package)

**Step 3: Verify project compiles**
```bash
dotnet build YourGame.csproj
```

**Important:** Your app code only needs:
```csharp
...
The `NhemDangFugBixs.Runtime` DLL is optional - only needed if you use runtime helpers.

---

### NDFG002: Version Mismatch

**Warning Message:**
```
Assembly version mismatch detected for NhemDangFugBixs attributes. Expected {0}, found {1}.
```

**Solution:** Re-import package to ensure all components are the same version.

---

### NDFG003: Unsupported Asmdef Setup

**Info Message:**
```
The generator encountered an unsupported Unity assembly setup: {0}
```

**Solution:** Check your asmdef configuration and package layout.

---

### NDFG004: Generator Init Failed

**Error Message:**
```
NhemDangFugBixs generator failed to initialize: {0}. Code generation will be skipped.
```

**Problem:** Generator threw an exception during initialization. This is a bug in the generator.

**Solution:**
1. Check Unity console for detailed error
2. Try re-importing the package
3. If persists, [report issue](https://github.com/NhomNhem/NhemDangFugBixs.Tooling/issues)

**Note:** When generator fails, project should still compile (graceful degradation).
